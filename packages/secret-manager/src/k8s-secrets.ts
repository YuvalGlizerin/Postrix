...
const secrets = Object.fromEntries(Object.entries(process.env).filter(([key]) => key?.startsWith('SECRET_'))) as Record<
  string,
  string
>;
const secretsToLoad = Object.entries(process.env).filter(([, value]) => value?.startsWith('/k8s-secrets/'));
...
const secretPromises = secretsToLoad.map(async ([key, value]) => {
  try {
